...
class SmsUtility
{
    ...
    public static function mobile_registration_verification($phone, $code)
    {
        $sms_body   = get_sms_template('mobile_registration_verification','sms_body');
        $sms_body   = str_replace('[[code]]', $code, $sms_body);
        $sms_body   = str_replace('[[site_name]]', get_setting('website_name'), $sms_body);
        $template_id= get_sms_template('mobile_registration_verification','template_id');
        if (get_setting('fast2sms_activation') == 1){
