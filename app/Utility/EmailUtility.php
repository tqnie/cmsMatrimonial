...
class EmailUtility
{
    ...
    // Email verification for user Registration
     public static function email_verification_for_registration_user($identifier, $email, $verificationCode){
        $user = '';
        $emailSubject  = get_email_template($identifier,'subject');
        $subject = str_replace('[[site_name]]', get_setting('website_name'), $emailSubject);
        $email_body = get_email_template($identifier,'body');
        ...
        return response()->json(['status' => 0, 'message' => translate('Failed to send verification code.')]);
    }
